...
function isDateInPeriod(date, period) {
  ...
  return dateToCheck >= startPeriod && dateToCheck <= endPeriod;
}
...
function formatDate(date) {
  const newDate = new Intl.DateTimeFormat('en-US', { timeZone: 'UTC' }).format(
    new Date(date)
  );
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = 1; day <= numberOfDays; day += 1) {
    const dayOfMonth = new Date(year, month - 1, day);
...
function getWeekNumberByDate(date) {
  ...
  const dayOfWeekCurrentDay = date.getDay();
  ...
  if (dayOfWeekCurrentDay === 0) {
    weekNumber -= 1;
  }
  return dayOfWeekFirstDay > 0 ? weekNumber : weekNumber + 1;
}
...
function getNextFridayThe13th(date) {
  let tempFriday = getNextFriday(date);
  while (tempFriday.getDate() !== 13) {
    tempFriday = getNextFriday(tempFriday);
  }
  return tempFriday;
}
